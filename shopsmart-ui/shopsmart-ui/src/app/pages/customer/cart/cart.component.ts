...
export class CartComponent implements OnInit, OnDestroy {
  ...
  checkout(): void {
    ...
    this.orderService.createOrderFromCart(customerId).subscribe({
      next: (response: any) => {
        console.log('Order created successfully from cart:', response);
        this.processingCheckout = false;
        this.cartUpdateService.notifyCartChanged();

        if (response && response.orderId) {
          this.router.navigate(['/home/checkout', response.orderId]);
        } else {
          this.cartError = 'Order created but ID was not returned by backend.';
        }
      },
      error: (error: HttpErrorResponse) => {
        this.processingCheckout = false;
